...
class VinoModelTest(TestCase):
    ...
    def test_vino_nombre_vacio(self):
        vino = Vino(nombre='', bodega='Solares del Norte', varietal='Malbec Syrah', region='Cuyo')  # Crea una instancia de Vino con nombre vacío
        with self.assertRaises(ValidationError) as context:  # Espera que se lance un Val|idationError
            vino.clean()  # Llama al método clean para validar|
        # Verifica que el mensaje de error sea el esperado
        self.assertEqual(context.exception.messages, ['El campo nombre no puede estar vacío.'])

    # Prueba que el campo 'region' no puede estar vacío
    def test_vino_region_vacio(self):
        vino = Vino(nombre='Malbec', bodega='Catena Zapata',varietal='Tempranillo', region='')  # Crea una instancia de Vino con region vacío
        with self.assertRaises(ValidationError) as context:
            vino.clean()  # Llama al método clean
        # Verifica que el mensaje de error sea el esperado
        self.assertEqual(context.exception.messages, ['El campo region no puede estar vacío.'])

    # Prueba que un vino válido no lance errores de validación
    def test_vino_correcto(self):
        vino = Vino(nombre='Malbec', bodega='Los Haroldos', varietal='Rosado', region='Mendoza')  # Crea una instancia válida
        try:
